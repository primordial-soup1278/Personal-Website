...
const ProjectCard = ({ title, description, tags, imageUrl, link }: Project) => {
  return (
    <a
      href={link}
      target="_blank"
    ...
    >
      <div className="relative min-w-[400px] max-w-[400px] bg-white/5 border border-white/10 rounded-2xl p-4
        ...
        <div className="flex gap-2 flex-wrap">
          {tags.map((tag) => (
            <span
              key={tag}
              className="text-xs font-medium px-2 py-1 rounded-md bg-white/10 text-blue-300"
            >
              {tag}
            </span>
          ))}
